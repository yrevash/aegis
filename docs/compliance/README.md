...
**Total: 114 controls — 36 enforced · 54 partial · 19 not implemented · 5 not
...
**One framework is now enforced in every mapped control: NIST AI RMF, all four
functions** — see section 9 and `docs/governance/`, the written artefacts that
close Govern and Map.
...
| **AML.T0024** Exfiltration via AI Inference API (model extraction) | **partial** | The **channel** is closed and measured: `aegis/src/aegis/guardrails/schema.py::exfiltration_channel` blocks an answer that carries data out through a URL nobody clicked — an auto-loading markdown or HTML image, or a link, pointing at an external host with an encoded payload in its query, path or fragment. Three probes exercise it; an ordinary documentation link is not a false positive. **What still leaks:** extraction by query volume. Model extraction by systematic scoring requests, and membership inference over a list of record ids, pass every rail and are reported as leaks — nothing in a text screen can tell the 500th probing question from the first legitimate one. What bounds them is the gateway's RPM/TPM/USD caps (claimed under LLM10), which is a bound, not a detection: there is no per-principal query-pattern analysis anywhere in this platform. |
...
**Coverage: 8 enforced · 1 partial · 0 not implemented · 1 not applicable.**
