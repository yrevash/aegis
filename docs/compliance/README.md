...
**Total: 114 controls — 29 enforced · 58 partial · 22 not implemented · 5 not
...
| **MeitY IAGG** | Risk mitigation by design | **partial** | Guardrails on all three rails; a 28-attack battery with 8 benign controls so the block rate is quoted beside a false-positive rate; an inherent-versus-residual risk map where no risk is ever marked solved. Test: `aegis/tests/redteam/test_redteam.py::test_control_category_blockrate_is_false_positive_rate`. | The hazard taxonomy is MLCommons and OWASP. No India-specific harm set, no Indian-language adversarial probe, no evaluation against the deepfake and synthetic-content harms the Guidelines emphasise. |
...
|---|---|---|---|---|
| **LLM01** | Prompt Injection | **enforced** | `aegis/src/aegis/guardrails/classifier.py` (`deterministic_injection` signature backstop + fail-closed `classify_injection`); screened at all three rails — `aegis/src/aegis/guardrails/pipeline.py` (`check_input`, `check_output`, `check_tool_result`); indirect injection tested at the rail it would actually arrive on. 11 of 28 attack probes carry `owasp="LLM01"`. Tests: `aegis/tests/redteam/test_stages_and_suites.py::test_the_indirect_injections_are_caught_at_the_tool_result_rail`, `aegis/tests/security/test_posture.py::test_injection_enforced_when_model_layer_wired`. | Degrades to **partial** when no model completer is wired — 5 battery probes are semantic-only and leak offline **by design**, and the report says so rather than hiding it. Injection is never marked solved (`backend/src/app/platform/risk_map.py`, AA-03). |
...
| **LLM03** | Supply Chain | **partial** | Fully hash-pinned lockfiles: `backend/uv.lock` (319 distributions, 4219 `sha256` digests) and `aegis/uv.lock` (129). A live SBOM resolved from the **actually installed** distributions at request time, never a hand-written pin list: `backend/src/app/platform/stack.py` → `GET /v1/stack`. Registry-verified freshness with an explicit honesty rule ("no clean bill of health without a real answer"): `backend/src/app/platform/patches.py` → `POST /v1/stack/patch-check`. One vetted model gateway rather than many provider SDKs (ADR `docs/adr/0001-litellm-as-gateway.md`). Tests: `backend/tests/api/test_platform_surfaces.py`. | **No CVE/OSV advisory feed** — freshness is *installed vs latest*, which is a proxy for patch posture, not a vulnerability verdict. No CycloneDX/SPDX export. No signature or provenance attestation (Sigstore/SLSA). A CI pipeline now exists (`.github/workflows/ci.yml`: ruff, the backend and aegis suites against a real PostgreSQL service, `tsc --noEmit`, the web suite and the OpenAPI snapshot check), so a merge is gated — but it gates **tests and lint, not supply chain**: none of the advisory, SBOM-export or attestation gaps above is checked by it. |
| **LLM04** | Data & Model Poisoning | **partial** | Retrieval side is gated at write time: `aegis/src/aegis/retrieval/validation.py::validate_content` rejects embedded injection payloads, oversized and non-printable blobs *before* the store. | **Scope note:** `/v1/security/posture` reports LLM04 `enforced` for the control it names, and that is correct for the ingestion gate. The framework's control is wider: Aegis's own ML spine trains from a host-supplied frame (`aegis/src/aegis/ml/dataset.py`) with **no provenance attestation on the training data**, and no dataset integrity hash. Partial at the framework level. |
...
| **LLM08** | Vector & Embedding Weaknesses | **partial** | Both ends of RAG are defended: `aegis/src/aegis/retrieval/spotlight.py::build_spotlighted_context` fences and datamarks retrieved spans as untrusted DATA (Microsoft Spotlighting, arXiv:2403.14720), and `validate_content` screens every write. | **Scope note:** posture reports `enforced` for those two mechanisms and that is right. The vector tier itself is a shared Qdrant node (`backend/src/app/platform/stack.py`) that sits **outside Postgres RLS** — tenant separation there is application-scoped, not database-enforced, so the framework row is partial. |
| **LLM09** | Misinformation | **partial** | `aegis/src/aegis/guardrails/grounding.py::check_grounding` exists but is **advisory by default** (flags; blocks only when `grounding_block`). Backed by conformal abstention bands (ADR `docs/adr/0007`) and a per-metric eval regression gate (`aegis/src/aegis/evals/regression.py`). Content-safety hazard screening (`aegis/src/aegis/guardrails/content_safety.py`) covers 6 of the battery's probes. Test: `aegis/tests/security/test_posture.py::test_misinformation_is_honestly_partial`. | The grounding rail does not block unless a deployment turns it on. Stated, not hidden. |
| **LLM10** | Unbounded Consumption | **partial** | The self-repair loop is *always* hard-capped (`AgentConfig.max_plan_iterations` — guaranteed termination). Token/USD/RPM/TPM caps are enforced **before spend** at the single gateway chokepoint (`aegis/src/aegis/governance/enforcement.py::enforce_governance`), with a 429 admission gate on job submission. Tests: `aegis/tests/security/test_posture.py::test_consumption_enforced_with_budget_hook`, `::test_consumption_partial_when_budget_fail_open`. | Spend caps bind only when the host injects a governance hook; `GATEWAY_BUDGET_FAIL_OPEN=true` makes a governance read failure pass the call through. Both are reported honestly by the live posture. |

**Coverage: 5 enforced · 5 partial · 0 not implemented.**
...
all real and none of them was labelled as supply-chain assurance. It is now
labelled, surfaced and tested. It stays **partial**, because inventory plus
freshness is not vulnerability management.
...
| **A03:2025** | Software Supply Chain Failures | **partial** | Same evidence as LLM03 above, plus the merge gate in `.github/workflows/ci.yml`. | Same gaps as LLM03: no advisory feed, no attestation. CI gates tests and lint; **nothing in it scans dependencies or produces provenance**. |
...
Only techniques the **battery actually exercises** are claimed. 36 probes: 28
attacks and 8 benign controls that measure the false-positive rate — a block rate
quoted without one is the number a vendor quotes.
...
| **AML.T0053** LLM Plugin Compromise | **partial** | 3 `excessive_agency` probes plus the real control — external MCP tools are gated HIGH by default and stop at the human gate (`backend/src/app/mcp/server.py`). The probes test the rail's refusal, not a compromised peer end to end. |
| **AML.T0020** Poison Training Data | **not_implemented** | The battery has no training-data attack; see LLM04. |
| **AML.T0024** Exfiltration via AI Inference API (model extraction) | **not_implemented** | No extraction or membership-inference probes. |
| **AML.T0043** Craft Adversarial Data | **not_implemented** | No evasion/perturbation probes against the ML spine. |
...
**Coverage: 5 enforced · 1 partial · 3 not implemented · 1 not applicable.**
...
|---|---|---|---|
| **GOVERN** | **partial** | Nine ADRs recording decisions and their rejected alternatives (`docs/adr/`); a written threat model; a per-tenant policy catalogue with explicit merge rules and who may write each setting (`aegis/src/aegis/settings/spec.py`); RBAC and named-seat grants. | No written AI policy, no accountable-role register, no incident-response plan, no periodic review cadence. |
| **MAP** | **partial** | `GET /v1/risk-map` places each agentic risk at an inherent **and** residual point with the control that moved it and a `control_ref` naming a real file; `GET /v1/stack` is the system inventory; `GET /v1/ml/model-card` states task, features, data source, calibration and training sizes; `aegis.pipelines` declares each flow's stages and is verified against the code before it is served. | No context-of-use or affected-persons analysis; no impact assessment on individuals. |
...
- NIST AI RMF **Measure** and **Manage** are genuinely strong; **Govern** is
...
- No incident response, no backup/restore, no DPIA, no ROPA, no QMS.
