...
| 6 | A model outside the platform's allowed deployments | The picker enumerates `tenant_model_choices()` — the tenant-selectable half of `_FLEET_DECLARATION`, which `routing_table()` could never supply because it maps a role to *one* deployment, not to a set of alternatives. **The server validates the override against that same function**, on write via `agent.model`'s `choices_source` and again at the point of use in `_resolve_model_choice`. A UI enum is not enforcement — here it is a projection of the enforcement |
