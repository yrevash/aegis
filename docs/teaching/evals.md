...
They are **proxies**: this gate makes no model call, touches no network, and does not
import `ragas`.

That is a statement about **this gate**, not about the repo. `ragas>=0.4.3` *is* a
dependency of Aegis, and `aegis.evals.libs.ragas_suite` runs the real library's metrics
through the platform's own metered gateway — see *Live scoring* below. The two exist for
different jobs: the offline gate has to be free, deterministic and runnable in CI with no
keys, and a judged metric is none of those things.
...
## Live scoring — the real RAGAS library

Everything above is the **offline gate**: free, deterministic, no network. Beside it sits
a second path that runs the actual `ragas` package.

**It is opt-in, and it costs money.** `POST /v1/evals/live-run` is a POST and is not
memoised, because every metric is LLM-judged and answer relevancy also embeds. Measured:
about **9 gateway calls per case** — 5 completions and 4 embeddings. `GET /evals/report`
stays the cheap deterministic rollup a dashboard may poll; conflating the two would turn
a page refresh into a budget event. The button on the evals screen states the cost before
you press it.

**Every judge call goes through the Aegis gateway, not at a provider directly.** That is
the whole reason `aegis/src/aegis/evals/libs/gateway_adapters.py` exists. Pointing `ragas`
at a `base_url` would work in ten minutes and would route every judge call *around*
budget checks, the usage ledger, rate limiting and tracing — making the evaluation
subsystem the one place this platform's metering claim is false. It shipped that way
once: seven invocations, ~108 model calls, ~$0.088 spent, **zero `usage_ledger` rows**,
because the route bound no `GovernanceContext`. The adapters were always right; the route
was the hole.

**Two of the library's own outputs are not measurements.** `Faithfulness` returns `NaN`
when the judge produced no statements, and `AnswerRelevancy` returns `0.0` when it
generated no question to compare against. `NaN` is not `None`, so one such case turned an
entire metric `NaN` while the panel still claimed two cases contributed; a judge-failure
zero drags a corpus mean down with a number nobody measured. `_usable()` treats both as
not-run, and a partial sample says so on the panel.

**The panel states its own setup.** Faithfulness reads 1.000 because the "answer" under
test *is* the retrieved context — true by construction. That measures whether the metrics
run end to end against real content, not whether a generated answer is good, and the card
says so above the figure rather than letting the number speak for itself.

**DeepEval is not installed, and the reason is specific.** `deepeval` requires
`click>=8.0.0,<8.4.0`; `huggingface_hub` requires `click>=8.4.2`. The ranges are disjoint,
so the two cannot coexist in one interpreter — and `uv pip install deepeval` *appears* to
succeed while silently downgrading click and leaving `huggingface_hub` violating its own
pin. The offline gate borrows DeepEval's *shape* (a declarative `Metric` carrying its own
pass bar) and is labelled a pattern, not the library.
