...
:mod:`aegis.ops.release` is deliberately abstract: it takes an ``eval_fn`` (score a
...
The host-owned pieces are **injected** via :func:`aegis.ops.config.configure_ops`: the
durable-approval writer (``enqueue_approval``), the session factory + ``set_tenant_scope``
binder for the gate's own short-lived read/decide sessions, and the ``Approval`` ORM class
+ its status enum (agent-owned; the gate reads/decides rows via the injected session but
never owns the table). ``RiskLevel`` is the shared ``aegis.core.types`` contract.
...
* :func:`enqueue_release_approval` writes a durable host ``Approval`` row
  (``action="prompt_release"``), reusing the same approvals table the agent gate uses, so
  a staged release is a real pending inbox item — but with a synthetic ``run_id``/
  ``thread_id`` so it is fully **decoupled** from the agent-run resume machinery (no
  checkpoint, no live socket).
* :func:`list_pending_releases` / :func:`decide_release` are the inbox read + resolve
  path: a decision calls :func:`aegis.ops.release.apply_release_decision` (promote or
...
from aegis.ops import config
from aegis.ops.models import PromptVersion
...
def make_eval_fn(complete: Any, *, limit: int = DEFAULT_EVAL_SUBSET) -> Any:  # noqa: ANN401
    ...
    :data:`~aegis.evals.corpus.SEED_CASES` it:

    1. retrieves context with the offline hybrid retriever
       (:func:`aegis.evals.harness.build_eval_retriever` — the same real fused pipeline the
       CI gate runs);
    ...
    3. grades that answer for groundedness + relevance with the reasoning-model judge
       (:func:`aegis.evals.judge.judge_answer`).
    ...
    :func:`aegis.ops.release.release` compares a *real* eval of the draft against a *real*
    eval of the baseline, never a constant.

    Args:
        complete: The chat-completion callable (``the injected complete`` in production;
...
async def enqueue_release_approval(
    *,
    ...
    enqueue_approval: Any = None,  # noqa: ANN401 - injected async approval writer
) -> str:
    """Persist a durable ``PENDING`` approval for a staged prompt release; return its id.

    Reuses the host ``Approval`` table (``action="prompt_release"``) so a staged release is
    a first-class, restart-surviving inbox item resolved by :func:`decide_release`. It is
    intentionally **decoupled** from the agent-run resume machinery: the synthetic
    ``run_id``/``thread_id`` (``prompt_release:<draft_id>``) is not a LangGraph thread, so
    no checkpoint or live socket is ever involved.

    This is the ``approval_enqueue`` injected into :func:`aegis.ops.release.release`.
    ...
    Args:
        ...
        enqueue_approval: Optional override for the durable approval writer; defaults to
            the one configured via :func:`aegis.ops.config.configure_ops`.

    Returns:
        The new approval's id.
    """
    if enqueue_approval is None:
        enqueue_approval = config.get_enqueue_approval()
...
async def list_pending_releases(
    *, limit: int = 50, tenant_id: int | None = None
    ...
    """
    approval = config.get_approval_model()
    approval_status = config.get_approval_status()

    async with config.get_session_factory()() as session:
        await config.apply_tenant_scope(session, tenant_id)
        stmt = (
            select(approval)
            .where(
...
async def decide_release(
    *, approval_id: str, approved: bool, decided_by: str | None = None
    ...
    :func:`aegis.ops.release.apply_release_decision` (promote on approve, archive on
