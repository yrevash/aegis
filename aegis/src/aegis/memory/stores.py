...
primary, NULL-safe, dialect-independent isolator. ``tenant_id`` is a plain indexed column
(no cross-package foreign key): the tenancy table is a host concern, so a host wires
Postgres RLS additively if it wants a DB-enforced belt. A NULL-tolerant RLS policy is a
separate, opt-in step (as bootstrapped, the standard per-tenant policy fails closed on a
NULL ``tenant_id``), so these tables are deliberately isolated at the query layer.
...
class MemoryWriteLog(AegisBase):
...
class MemoryConsolidationJob(AegisBase):
