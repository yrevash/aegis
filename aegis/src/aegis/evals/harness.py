...
(:class:`~aegis.retrieval.pipeline.Retriever` over the databaseless
:class:`~aegis.retrieval.memory.InMemoryKnowledgeBackend`) across the fixed
:data:`~aegis.evals.corpus.SEED_CASES`, scores each with the deterministic
:mod:`~aegis.evals.metrics`, and returns an :class:`EvalReport` whose ``passed`` flag is
...
def build_eval_retriever() -> Retriever:
    ...
    Uses the production :class:`~aegis.retrieval.pipeline.Retriever` and the real
    :class:`~aegis.retrieval.memory.InMemoryKnowledgeBackend` (genuine vector + graph +
...
async def evaluate(
    ...
    **LLM-as-judge** (:func:`aegis.evals.judge.judge_answer`) additionally grades each case's
    ...
    Args:
        ...
        complete: Optional chat-completion callable (shape of a gateway ``complete``);
            when given, the LLM-as-judge runs through it.
