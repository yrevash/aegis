...
1. **Ask the cache first.** A rehearsed query on a warm cache costs zero provider
   ...
   conference wifi and a rate limit.
2. **Screen every hit through the ``TOOL_RESULT`` rail** before it is returned to
   anyone. Search results are arbitrary third-party text that a model reads as
   context — OWASP LLM01 — and this is the only place in the turn that looks at them.
   A blocked hit is *reported*, not quietly dropped.
...
class WebSearch:
    ...
    async def search(
        ...
        *,
        ...
        if cached is not None:
            response = cached.model_copy(update={"query": query, "cached": True})
        ...
        response = await self._screen(query, hits)
        self._cache_set(key, response)
        await self._emit(emitter, response)
        return response

    async def _screen(
        self, query: str, hits: list[WebSearchResult]
    ) -> WebSearchResponse:
        """Run every hit through the ``TOOL_RESULT`` rail before it can reach context."""
        ...
        if self._guard is None:
            ...
            return WebSearchResponse(query=query, results=tuple(hits), provider=provider)
    ...
    def _cache_get(self, key: str) -> WebSearchResponse | None:
        """Read a cached response, failing open (a miss) on any error or corruption."""
        try:
        ...
        if raw is None:
            return None
        try:
            return WebSearchResponse.model_validate_json(raw)
    ...
    def _cache_set(self, key: str, response: WebSearchResponse) -> None:
        """Store the *screened* response, swallowing any error (fail-open).

        The screened response is what is cached, not the provider's raw payload: a
        warm cache must cost zero provider calls **and** zero classifier calls, and
        the rail's verdict on a fixed string does not change between runs. The
        ``blocked`` list rides along, so a cached answer still reports the injection
        it caught.
        """
        try:
            self._cache.set(key, response.model_dump_json(), ttl=self._ttl)
