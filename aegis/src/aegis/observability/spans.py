...
and embedding calls. It is the counterpart to :func:`aegis.observability.genai_span`
(which owns the ``gen_ai.*`` LLM/embedding spans).

Everything degrades to a **no-op** when no tracer/Phoenix is configured: before
:func:`aegis.observability.init_observability` runs (or in offline/lite mode and
in tests), :func:`aegis.observability.get_tracer` resolves against OTel's global
