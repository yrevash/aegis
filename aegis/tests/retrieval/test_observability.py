...
_DOCS = [
    ...
    ("escalation", "Escalate a refund request to a senior agent when its deadline is at risk."),
    ("payment", "A payment method can be a card or a bank transfer for a refund."),
]
...
async def test_query_rewrite_reported_with_rewritten_query():
    ...
    assert rw.rewritten == "refund payment method expanded standalone"
    # The loop actually retrieved with the rewritten query (round 1).
    assert out.rounds[0].query == "refund payment method expanded standalone"
...
async def test_self_rag_iteration_count_reported_single_round():
    ...
    assert ag.round_queries == ["refund payment method"]
