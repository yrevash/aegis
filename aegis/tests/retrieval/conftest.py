...
class RecordingComplete:
    ...
    Each response also carries a `.usage` (mirroring a host's LLM result) so callers
    that accrue per-call token/cost — e.g. query rewrite — can be exercised. Consumers
    that read only `.content` are unaffected.
    """
