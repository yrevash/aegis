...
object: ``aegis.memory``'s recall built an in-process Chroma index on first use, and a
...
def test_retrieval_backend_refuses_to_invent_a_store(unconfigured: None) -> None:
    ...
    assert "path=" in message and "ChromaVectorStore.local" in message
...
def test_each_backend_gets_its_own_store_from_the_factory() -> None:
    ...
    """
    built: list[ChromaVectorStore] = []

    def factory() -> ChromaVectorStore:
...
def test_memory_index_refuses_to_conjure_itself(unconfigured: None) -> None:
    ...
    assert "path=" in message and "MemoryVectorIndex.local()" in message
