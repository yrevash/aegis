"""Unit tests for JWT access tokens, password hashing, and role derivation."""
...
def test_configure_security_rotates_the_signing_secret():
    # A token minted under one secret must not verify under a different one — proving
    # the injected SecurityConfig is what signs/validates (no host config import).
    configure_security("secret-alpha-0123456789abcdef0123456789abcdef")
    token = create_access_token(user_id=1, username="x", role=MEMBER, tenant_id=1)
    assert decode_access_token(token).username == "x"
    configure_security("secret-bravo-0123456789abcdef0123456789abcdef")
    with pytest.raises(jwt.InvalidTokenError):
        decode_access_token(token)
    # Restore the documented dev default so later tests in the process are unaffected.
    configure_security("dev-insecure-jwt-secret-change-me-0123456789abcdef")
