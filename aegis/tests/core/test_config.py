...
def test_default_mode_is_full() -> None:
    settings = CoreSettings(
        redis_url="r", database_url="d", vector_store_url="http://localhost:6333"
    )
    assert settings.mode is AegisMode.full


def test_full_mode_missing_infra_raises() -> None:
    s = CoreSettings(mode="full", redis_url=None, database_url=None, vector_store_url=None)
    with pytest.raises(RuntimeError) as ei:
        s.require_full_infra()
    assert "REDIS_URL" in str(ei.value) and "AEGIS_MODE=lite" in str(ei.value)


def test_full_mode_requires_the_vector_store_url() -> None:
    # The Qdrant node is a hard full-mode dependency: without an explicit URL the index
    # would be the ephemeral in-process engine, i.e. a silent degrade — and a
    # single-process one, which is the ceiling §9.1 removed.
    s = CoreSettings(mode="full", redis_url="r", database_url="d", vector_store_url=None)
    with pytest.raises(RuntimeError) as ei:
        s.require_full_infra()
    assert "AEGIS_VECTOR_STORE_URL" in str(ei.value)


def test_full_mode_with_all_infra_ok() -> None:
    CoreSettings(
        mode="full", redis_url="r", database_url="d", vector_store_url="http://localhost:6333"
...
def test_error_names_the_prefixed_env_vars() -> None:
    ...
    for name in ("AEGIS_REDIS_URL", "AEGIS_DATABASE_URL", "AEGIS_VECTOR_STORE_URL"):
...
async def test_auto_probes_and_resolves_to_full_when_every_backend_answers(monkeypatch) -> None:
    ...
    s = CoreSettings(
        mode="auto", redis_url="r", database_url="d", vector_store_url="http://localhost:6333"
    )
...
async def test_auto_without_urls_resolves_to_lite_and_says_so(caplog) -> None:
    ...
    s = CoreSettings(mode="auto", redis_url=None, database_url=None, vector_store_url=None)
    with caplog.at_level(logging.WARNING, logger="aegis.core.config"):
        assert await s.resolve_mode() is AegisMode.lite
