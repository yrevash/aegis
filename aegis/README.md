...
- **Honest infrastructure** — no silent fallbacks. A component either has the backing store it needs (Redis, Postgres, pgvector) or refuses to boot with a clear error. Lite mode is opt-in and loudly marked.
...
- Typed config with explicit `AEGIS_MODE`:
  - `full` (default) probes Redis, Postgres, and pgvector at boot; refuses to start if any required backend is missing.
