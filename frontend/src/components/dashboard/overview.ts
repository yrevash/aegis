/**
 * Pure derivations for the Overview surface (§4.2). Kept recharts-free and
 * side-effect-free so every headline figure is provably a function of the real
 * `GET /metrics` payload and can be unit-tested in isolation (see
 * `overview.test.ts`). The React tiles only format + animate these values.
 *
