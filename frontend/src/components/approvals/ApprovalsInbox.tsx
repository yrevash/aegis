...
import { getApprovals, postApprovalDecision } from '@/api/client'
import { DonutChart, type DonutDatum } from '@/components/charts/DonutChart'
import { BentoGrid, BentoTile, CountUp } from '@/components/shared'
import { InfoTip } from '@/components/ui/InfoTip'
import type { ApprovalDecision, ApprovalRow } from '@/types/api'
...
/**
 * The **Approvals** surface (Aegis Tools/MCP, §4.5) — a calm triage board for
 * the durable human gate. A summary hero reads the queue state (pending,
 * overdue, average wait, risk mix) at a glance; below it, a responsive card grid
 * shows each request with its action, risk, SLA and confidence, and a clear
 * approve / reject. The ML internals live behind a per-card disclosure.
 */
export function ApprovalsInbox({ token }: ApprovalsInboxProps): ReactElement {
  ...
  const decide = async (row: ApprovalRow, decision: ApprovalDecision): Promise<void> => {
    ...
    try {
      ...
      if (res.accepted && load.status === 'ready') {
        // Fade the card out, then remove it from the queue (§2.5).
  ...
  return (
    <div className="space-y-6">
      {/* Queue-summary hero — read the queue state before scrolling (§4.5). */}
