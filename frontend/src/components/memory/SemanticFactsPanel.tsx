...
import type { MemoryFactRow, MemoryFactsResponse } from '@/types/memory'
  ...
  return (
    <li className={cn('rounded-lg border p-3', invalid ? 'border-border/60 bg-surface-2/30' : 'border-border bg-card')}>
      <div className="flex items-start gap-2">
        ...
        <Badge variant={status.tone === 'ok' ? 'ok' : 'secondary'} className="shrink-0 text-[0.56rem]">
          {status.label}
        </Badge>
      </div>
    ...
    <div className="flex h-full flex-col gap-3">
      <PanelHeader
        ...
        right={
          <>
            {state.status === 'ready' && (
              <Badge variant="graph" className="text-[0.58rem]">
                {validCount} current
              </Badge>
            )}
