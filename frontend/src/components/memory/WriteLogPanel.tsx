...
import { EmptyRow, ErrorRow, LoadingRow } from '@/components/common/StateRow'
import { Badge } from '@/components/ui/badge'
import { InfoTip } from '@/components/ui/InfoTip'
import { formatAgo } from '@/lib/datetime'
import type { AsyncState } from '@/components/admin/useAsync'
import type { MemoryWriteRow, MemoryWritesResponse } from '@/types/memory'

import { PanelHeader } from './PanelHeader'

type Variant = 'ok' | 'graph' | 'block' | 'secondary'

/** Op → plain verb + icon + badge tone (drops the ADD/UPDATE/INVALIDATE codes). */
const OP_STYLE: Record<string, { icon: LucideIcon; variant: Variant; label: string }> = {
  ADD: { icon: FilePlus2, variant: 'ok', label: 'Learned' },
  UPDATE: { icon: PencilLine, variant: 'graph', label: 'Updated' },
  INVALIDATE: { icon: FileX2, variant: 'block', label: 'Retired' },
  NOOP: { icon: PencilLine, variant: 'secondary', label: 'Reviewed' },
}
...
function Delta({ row }: { row: MemoryWriteRow }): ReactElement | null {
  ...
  return (
    <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 font-mono text-[0.62rem]">
      {keys.map((k) => {
 ...
 * (learned / updated / retired), with the reason, the model, and the trace id.
 * The append-only guarantee is kept as a small badge with the honest detail in
 * its ⓘ. (Underneath: the append-only memory write-log.)
 */
export function WriteLogPanel({ state }: Props): ReactElement {
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      <PanelHeader
        ...
        right={
          <span className="inline-flex items-center gap-1">
            <Badge variant="secondary" className="text-[0.56rem]">
              append-only
            </Badge>
      ...
      {state.status === 'ready' && rows.length > 0 && (
        <ol className="relative flex-1 space-y-3.5 before:absolute before:top-2 before:bottom-2 before:left-[9px] before:w-px before:bg-border/70">
          {rows.map((row, i) => {
              ...
              <RevealOnScroll key={row.id} delayMs={i * 40}>
                <li className="relative flex gap-3 pl-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {row.reason && <p className="mt-1 text-xs leading-snug text-foreground">{row.reason}</p>}
                    <Delta row={row} />
