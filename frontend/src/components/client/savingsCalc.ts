/**
 * Pure derivation + formatting logic for the client Savings view (§ value).
 *
 * Side-effect free and recharts-free so it is unit-testable in a plain Node
 * environment (see `savingsCalc.test.ts`); `SavingsView.tsx` only renders what
 * these functions derive. The point is to *show the derivation* — the headline
 * saving reconciled against the itemised breakdown, not a bare number.
 */

import type { SavingsBreakdownRow, SavingsResponse } from '@/types/api'
