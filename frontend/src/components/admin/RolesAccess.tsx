...
import { assignUserRole, getUsers } from '@/api/client'
 ...
 * admin can never accidentally strip their own (or the last) admin access. The
 * pure catalog / counts / guard live in `roleCatalog.ts` and are unit-tested.
 */
...
export function RolesAccess({ token }: { token: string | null }): ReactElement {
  ...
  const reassign = (user: AdminUser, target: Role): void => {
    ...
    // disabled). Never let an admin strip their own / the last admin access.
    const guard = roleOptionGuard({ user, target, currentUsername, users: load.rows })
    if (guard.disabled) {
      toast.error('Change blocked', { description: guard.reason ?? undefined })
      return
    }
    ...
    void assignUserRole(user.id, target, token).then(
      (updated) => {
        ...
        toast.success('Role updated', {
          description: `${user.username} → ${ROLE_CATALOG[target].label}`,
        })
      },
      (e: unknown) => {
        ...
        toast.error('Could not update role', {
          description: e instanceof Error ? e.message : `${user.username} kept ${ROLE_CATALOG[normalizeRole(user.role)].label}`,
        })
      },
    )
  }
