...
export const CHAIN: readonly ChainLink[] = [
  ...
  },
  {
    ...
    label: 'Input rails',
    node: 'guard_input',
    note: 'Six checks run before a single token is spent, and the run names the one that fired rather than reporting a generic block.',
  ...
  },
  {
    ...
    label: 'Route',
    node: 'route',
    note: 'A supervisor classifies the turn, picks the specialist and sizes the team — then reports who decided, including the tenant ceiling that clamped it.',
  ...
  },
  {
    ...
    label: 'The team',
    node: 'run_team',
    note: 'Up to four specialists run at once, each streaming its own reasoning. Three of the four hold no write tool at all — a read-only remit does not acquire one by being on a team.',
  ...
  },
  {
    ...
    label: 'Synthesis',
    node: 'synthesize',
    note: 'The lanes run on the cheap model. The expensive one is spent once, here — which is most of why fanning out four ways is affordable at all.',
  ...
  },
  {
    ...
    label: 'Output rails',
    node: 'guard_output',
    note: 'The answer is checked on the way out as well as the way in — its shape, its grounding in what was retrieved, and the tenant’s own topic policy.',
  ...
  },
  {
    ...
    label: 'Answer',
    node: 'stream',
    note: 'Every figure arrives with its origin attached. A figure that cannot be sourced is stated as absent, in the slot the number would have occupied.',
