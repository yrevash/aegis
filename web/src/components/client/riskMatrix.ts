/**
 * Pure placement + scoring logic for the client Risk Map.
 *
 * Side-effect free and recharts-free so it is unit-testable in a plain Node
 * environment; `RiskMap.tsx` only renders what these functions derive. The
 * matrix reads a likelihood(1..5) × impact(1..5) scale and plots each risk into
 * its cell, so the exposure is legible at a glance — top-right (high likelihood
 * + high impact) is the worst corner.
 */
