...
await login('devops')
for (const [path, name] of [['/app/devops/interop','interop'],['/app/devops/redteam','redteam-collapsed'],['/app/devops/compliance','compliance-enforced']]) {
  await p.goto(BASE + path, { waitUntil: 'domcontentloaded' }); await p.waitForTimeout(4500)
  await p.screenshot({ path: `/Users/yrevash/aegis/.demo/frames/${name}.png`, fullPage: false })
