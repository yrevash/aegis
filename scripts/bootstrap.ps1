...
.SYNOPSIS
  ...
  and frontend dependency, sets up the .env files, and reports what's ready.
.DESCRIPTION
...
$node = Have 'node';  Line $node "node    (need >= 18)"
$pnpm = Have 'pnpm';  Line $pnpm "pnpm    (npm i -g pnpm)"
$uv = Have 'uv';      Line $uv   "uv      (pip install uv)"
if (-not ($py -and $node -and $pnpm -and $uv)) {
...
$extras = 'data,observability,agent,retrieval,ml,guardrails,dev'
...
# ── Frontend ─────────────────────────────────────────────────────────────────
Write-Host "`n== Frontend ==" -f Cyan
Set-Location "$root\frontend"
pnpm install | Out-Null
if (-not (Test-Path '.env.local')) { Copy-Item '.env.example' '.env.local'; Write-Host "  created frontend\.env.local" -f Yellow }
Line $true "frontend ready"
