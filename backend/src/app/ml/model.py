...
comes from the adapter spec; *how* to predict, calibrate and explain lives here.
...
from app.ml.dataset import resolve_training_frame
from app.ml.spec import ResolvedSpec, TaskType, resolve_spec

if TYPE_CHECKING:
    from app.ml.spec import MLSpec
...
class TrustworthyModel:
    ...
    def predict_explain(self, features: dict[str, Any]) -> MLExplainResponse:
        ...
        Returns:
            An :class:`~app.api.schemas.MLExplainResponse` carrying the point
    ...
    def _attributions(self, x: pd.DataFrame, raw: pd.DataFrame) -> list[ShapFeature]:
        ...
        **aggregated back to the original features** (summing the one-hot columns of
        ...
        Returns:
            One :class:`~app.api.schemas.ShapFeature` per original feature, sorted by
