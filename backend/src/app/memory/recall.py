...
raw material the working-memory assembler (:mod:`app.memory.working`) later budgets and
...
:func:`app.memory.vector_ops.topk_by_cosine`, which is portable across the SQLite test DB
and Postgres. When ``query_vec`` is ``None`` (e.g. an exact-cache hit never computed one,
or a lite 256-dim vector is not recall-comparable) facts fall back to recency-only SQL.
"""
...
from app.memory.scoring import RecallCandidate, rank_top
from app.memory.stores import MemoryFact, MemoryMessage, MemoryProfile, MemorySession
...
class RecallBundle:
    ...
    Consumed by :func:`app.memory.working.assemble_working_memory`. Every field is
    already subject/tenant-scoped; the assembler only budgets, dedups, and orders it.

    Attributes:
        profile_text: The rendered "human block" (:func:`memory_spec.render_profile`), or "".
...
async def _recall_profile(
    ...
    *,
    ...
    if profile is None:
        return ""
    return memory_spec.render_profile(profile.data or {})
...
def _recall_skills(query: str, persona: str | None, config: MemoryConfig) -> list[tuple[str, str]]:
    """Select procedural skills for the query and read their markdown bodies."""
    try:
        available = [
            f[:-3] for f in os.listdir(memory_spec.SKILLS_DIR) if f.endswith(".md")
        ]
    except OSError:
        return []
    selected = memory_spec.select_skills(query, persona, available) or []
    skills: list[tuple[str, str]] = []
    for name in selected[: config.n_skill]:
        path = os.path.join(memory_spec.SKILLS_DIR, f"{name}.md")
        try:
...
async def recall(
    ...
    *,
    ...
    profile_text = await _recall_profile(
        session, subject_id=subject_id, tenant_id=tenant_id
    )
    ...
    skills = _recall_skills(query, persona, config)
