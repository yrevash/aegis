...
The tables that belong to other modules stay here on the platform's own :class:`Base`:
``Approval`` (agent HITL), ``Chunk`` (retrieval), ``EvalResult`` (evals), and
``PromptVersion`` (LLM-Ops). Their ``tenant_id`` is a plain indexed column (no
cross-package foreign key to the now-separate ``aegis.governance`` ``tenants`` table —
mirroring how ``aegis.memory`` isolates at the query/RLS layer); the belt-and-suspenders
tenant scoping + Postgres RLS provide the isolation, not a DDL foreign key.
