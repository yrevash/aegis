...
``init_observability(app)`` wires an OpenTelemetry tracer provider that exports
...
back to a plain OTel SDK provider with a console exporter so the app still runs
...
def _build_fallback_provider() -> TracerProvider:
...
def init_observability(app: Any = None) -> None:  # noqa: ANN401 - FastAPI app, optional
    ...
    Safe to call once at startup. If ``settings.phoenix_enabled`` is false, or if
    Phoenix / its OTel bridge are unavailable, a console-exporting SDK provider is
    installed instead so tracing still works offline.
    ...
    if not settings.phoenix_enabled:
        _provider = _build_fallback_provider()
