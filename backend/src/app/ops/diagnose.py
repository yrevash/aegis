...
Gap 2). It reads the recent *failing* :class:`~app.data.models.EvalResult` rows that
:mod:`app.ops.trace_eval` wrote, tallies **which metrics fail most** (answer vs the
...
is written back **only as a DRAFT** :class:`~app.data.models.PromptVersion` — it never goes
live here; promotion is the sole responsibility of :mod:`app.ops.release` behind the eval
...
* **Adapter prompt is the floor.** When no active version exists for the key, the base
  prompt is the adapter default (``render_system_prompt(get_persona(...))``).
    ...
    *,
    ...
    Args:
        ...
        prompt_key: The prompt whose active version is the base to improve (also the
            persona id used for the adapter-default floor when nothing is active).
    ...
    # Base prompt: the active version's prompt, else the adapter default (the floor).
    active = await registry.get_active(session, prompt_key)
    if active is not None:
        base_prompt = active.system_prompt
        parent_version = active.version
    else:
        base_prompt = render_system_prompt(get_persona(prompt_key))
        parent_version = None
