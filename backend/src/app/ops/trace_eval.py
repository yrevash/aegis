...
``docs/HARNESS_LLMOPS_PLAN.md`` Gap 2 + Gap 4): after a run finishes, an
...
GUARDRAIL / CHAIN), writing one :class:`~app.data.models.EvalResult` row per
...
#: deterministic lexical proxies (mirrors ``app.eval.metrics`` normalisation so a
