...
`IngestReport` are the two contract types named in `docs/AGENT_BRIEF.md`; the rest are
internal-but-typed structures used between the recall, rerank, and assembly stages.

The graph delta reuses the **locked** `GraphNode` / `GraphEdge` schemas from
`app.api.schemas` (never redefined here) so the frontend viz can animate straight from
a retrieval result.
"""
...
class CacheProvenance(BaseModel):
    """Where a cache-served result actually came from (§4.3) — never silent.
...
class Provenance(BaseModel):
    """Per-result retrieval provenance: origins, fusion method, cache lineage (§4.3).
...
class RetrievalResult(BaseModel):
    ...
    Attributes:
        ...
        provenance: Per-result origin/fusion/cache lineage (§4.3). Empty by default.
        query_vec: The query embedding computed this turn, surfaced for the "free"
            ...
            of dim ``EMBED_DIM``. ``None`` on an exact/near cache hit (never computed)
            and in lite/256-dim mode (not recall-comparable), so a downstream write
            never stores a mismatched-dim vector.
        query_vec_dim: The dimensionality of the computed query embedding (even when
            ``query_vec`` is left ``None`` because it was not EMBED_DIM), or ``None``
            when no embedding was computed (cache hit).
    """
    ...
    query_vec: list[float] | None = Field(
        ...
        "EMBED_DIM gateway vector, else None (cache hit / lite mode).",
    )
