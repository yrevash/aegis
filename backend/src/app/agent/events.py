"""Builders for the streamed :data:`~app.api.schemas.StreamEvent` payloads.

Graph nodes never construct the wire model directly. Instead they emit *partial*
event dicts through the LangGraph stream writer, and the orchestrator stamps each
one with the run-scoped ``run_id`` and a monotonic ``seq`` before validating it
against the locked :data:`~app.api.schemas.StreamEvent` discriminated union.

Centralising the wire shape here keeps the sequence numbering monotonic across a
whole run (a single async consumer assigns it) and means the node code stays free
of bookkeeping. Every builder returns a plain ``dict`` whose ``type`` selects the
union variant; :func:`stamp` finishes the job.
"""
...
def provenance(prov: Any, *, cache_hit: bool) -> dict[str, Any]:  # noqa: ANN401 - Provenance duck-type
    ...
    Reads the P0-frozen shape on :class:`app.retrieval.models.Provenance` (``origins``,
