"""Unit tests for the LiteLLM gateway (app.core.llm).

These run with **no network and no litellm installed**: a fake ``litellm`` module
is injected into ``sys.modules`` before the lazy import in ``app.core.llm`` runs.
"""
...
from app.core.llm import complete, embed
...
def fake_litellm(monkeypatch):
    """Install a fresh fake ``litellm`` and reset the module's config latch."""
    fake = FakeLiteLLM()
...
async def test_complete_disables_tls_verify(fake_litellm):
    ...
    # settings.genailab_ssl_verify defaults to False → TLS verification disabled.
...
async def test_complete_cost_falls_back_to_token_estimate(fake_litellm, monkeypatch):
    """When LiteLLM has no price for the custom deployment, cost is estimated
    from tokens (never a misleading $0) — see AUDIT.md H2."""
    fake_litellm._response = _make_response(content="x")
    ...
    # Positive, token-derived estimate rather than the old hard-coded 0.0.
    assert result.usage.cost_usd > 0.0


def test_usage_tally_tracks_baseline_and_savings(monkeypatch):
...
def test_usage_tally_defaults_to_zero_savings(monkeypatch):
    monkeypatch.setattr(llm_mod, "_tally", llm_mod._UsageTally())
    tally = llm_mod.usage_tally()
...
async def test_complete_forwards_default_max_tokens_and_timeout(fake_litellm):
    ...
    call = fake_litellm.calls[0]
    assert call["max_tokens"] == get_settings().llm_max_output_tokens == 1024
    assert call["timeout"] == get_settings().llm_timeout_seconds
...
async def test_complete_timeout_becomes_handled_error(fake_litellm, monkeypatch):
    ...
    monkeypatch.setattr(get_settings(), "llm_timeout_seconds", 0.02)
