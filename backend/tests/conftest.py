    ...
    *,
    ...
    async def retrieve(query: str, *, scope: RetrievalScope) -> RetrievalResult:
        return RetrievalResult(
            ...
            # copies ``Candidate.metadata`` onto every ``Source``, and every recall arm
            # writes ``file_path`` into it. The second source carries no path on
            # purpose — a chunk whose provenance was never recorded is a real shape, and
            # it must reach the wire as an absence rather than as a guess.
            sources=[
                Source(
                    id="kb-1",
